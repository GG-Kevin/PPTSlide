...
5. 플레이스홀더 바깥에 텍스트박스를 둘 때도 글꼴은 `현대하모니 L`,
   문단 들여쓰기(`marL`/`indent`)와 불릿은 `format_spec.json` 의 `body_levels` 값을 그대로 쓴다.
6. 도해 좌표는 `tools/build_slide.py` 상단 상수로 관리한다. 코드 곳곳에 숫자를 흩뿌리지 않는다.

## 마무리
생성 후 `python3 tools/verify.py output/<이름>.pptx` 를 돌려 26개 항목이 모두 PASS 인지 확인하고,
