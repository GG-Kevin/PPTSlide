...
- **툴 5종** — 스펙 추출, 도형 XML 헬퍼, 슬라이드 빌드, PNG 렌더, 포맷 검증
- **완성 예시** `output/Introduction_개발일정.pptx` — 간트 로드맵 + 단계 카드 + 핵심 메시지
...
`tools/verify.py` 는 29개 항목을 본다.

- **구조(XML)** — 슬라이드 크기, 마스터/레이아웃 세트 유지, 본문 레이아웃 사용,
  제목·본문 플레이스홀더 사용, 제목 스타일 상속, 승인 폰트만 사용,
  모티프 바/페이지번호 영역 침범 없음, 도형이 본문 영역 안, 카드 겹침 없음,
  간트 기간 바가 월 축 범위 내, 슬라이드 경계 이탈 없음
